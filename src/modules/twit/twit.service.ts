...
export class TwitService {
    constructor(
        ...
        private readonly twitRepository: Repository<TwitEntity>,
        @InjectRepository(TwitHasTagEntity)
        private readonly twitHasTagRepository: Repository<TwitHasTagEntity>,
        @InjectRepository(TwitLikeEntity)
        private readonly twitLikeEntityRepository: Repository<TwitLikeEntity>,
    ...
    public async findOne(
        ...
        return this.twitRepository.findOne(condition);
    }
    ...
    public async create(
        ...
        const twit = await this.twitRepository.save(newTwit);
        if (createDto.tags) {
    ...
    public async save(twitEntity: TwitEntity): Promise<TwitEntity> {
        return this.twitRepository.save(twitEntity);
    }
    ...
    public async saveTags(twit: TwitEntity, tags: string): Promise<void> {
        ...
        for (const someTag of tagsArray) {
            try {
                let tag: TagEntity;
                const existTags = await this.tagService.find({
                    where: { name: someTag },
                });
                if (existTags[0]) {
                    tag = existTags[0];
